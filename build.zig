...
pub fn build(b: *std.Build) !void {
    ...
    nanovg_mod.addIncludePath(.{ .src_path = .{ .owner = b, .sub_path = "src" } });
    nanovg_mod.addIncludePath(.{ .src_path = .{ .owner = b, .sub_path = "lib/gl2/include" } });
    nanovg_mod.addCSourceFile(.{ .file = .{ .src_path = .{ .owner = b, .sub_path = "src/fontstash.c" } }, .flags = &.{ "-DFONS_NO_STDIO", "-fno-stack-protector" } });
    nanovg_mod.addCSourceFile(.{ .file = .{ .src_path = .{ .owner = b, .sub_path = "src/stb_image.c" } }, .flags = &.{ "-DSTBI_NO_STDIO", "-fno-stack-protector" } });
